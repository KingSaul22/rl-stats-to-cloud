    ...
    match command {
        ControlCommand::AllowUi => {
            ...
            let task = tokio::spawn(async move {
                run_ui_websocket_server(bind_addr, state_receiver, task_shutdown, task_ui_control)
                    .await;
            });
        ...
        ControlCommand::DisallowUi => {
            ...
            ControlReply::Ok {
                message: if was_running {
                    ...
                    "DisallowUi acknowledged. UI websocket server was already stopped.".to_string()
                },
            }
        }
